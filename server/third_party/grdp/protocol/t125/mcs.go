...
func (c *MCSClient) SetClientCoreData(width, height uint16, colorDepth int, perfFlags int) {
	c.clientCoreData.DesktopWidth = width
	c.clientCoreData.DesktopHeight = height
	c.clientCoreData.SetColorDepth(colorDepth)
	// c.clientCoreData.SetPerformanceFlags(perfFlags) // Not fully implemented yet
}
