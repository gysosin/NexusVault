...
func (data *ClientCoreData) SetColorDepth(depth int) {
	switch depth {
	case 8:
		data.ColorDepth = RNS_UD_COLOR_8BPP
		data.PostBeta2ColorDepth = RNS_UD_COLOR_8BPP
		data.HighColorDepth = HIGH_COLOR_8BPP
	case 15:
		data.ColorDepth = RNS_UD_COLOR_16BPP_555
		data.PostBeta2ColorDepth = RNS_UD_COLOR_16BPP_555
		data.HighColorDepth = HIGH_COLOR_15BPP
	case 16:
		data.ColorDepth = RNS_UD_COLOR_16BPP_565
		data.PostBeta2ColorDepth = RNS_UD_COLOR_16BPP_565
		data.HighColorDepth = HIGH_COLOR_16BPP
	case 24:
		data.ColorDepth = RNS_UD_COLOR_24BPP
		data.PostBeta2ColorDepth = RNS_UD_COLOR_24BPP
		data.HighColorDepth = HIGH_COLOR_24BPP
	case 32:
		data.ColorDepth = RNS_UD_COLOR_24BPP // 32-bit is negotiated as 24-bit with 32bpp support flag
		data.PostBeta2ColorDepth = RNS_UD_COLOR_24BPP
		data.HighColorDepth = HIGH_COLOR_24BPP
		data.SupportedColorDepths |= RNS_UD_32BPP_SUPPORT
		data.EarlyCapabilityFlags |= RNS_UD_CS_WANT_32BPP_SESSION
	}
}

func (data *ClientCoreData) SetPerformanceFlags(flags int) {
	// Map performance flags to EarlyCapabilityFlags if applicable
	// Note: Most performance flags are in the Performance Capability Set (TS_PERFORMANCE_CAPABILITYSET),
	// which is part of the General Capability Set, not ClientCoreData.
	// However, some flags like RNS_UD_CS_WANT_32BPP_SESSION are here.
	// For now, we will just ensure 32bpp is set if requested.
	// Real performance flags (wallpaper, theming) are in the capability sets exchange, which might be handled elsewhere.
	// Let's check if we can set them here or if we need to modify capability sets.
}
